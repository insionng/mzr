...
var jbImagesDialog = {
	...
	inProgress : function() {
		...
		this.timeoutStore = window.setTimeout(function(){
			document.getElementById("upload_additional_info").innerHTML = '本次上传耗时比常规情况要长.' + '<br />' + '可能发生错误导致上传中断.' + '<br /><a href="###" onClick="jbImagesDialog.showIframe()">' + '查看响应信息' + '</a>';
	...
	uploadFinish : function(result) {
		...
		else
		{
			document.getElementById("upload_in_progress").style.display = 'none';
			document.getElementById("upload_infobar").style.display = 'block';
			document.getElementById("upload_infobar").innerHTML = '上传完成';
