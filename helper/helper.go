...
func Pagesbar(url string, keyword string, results_max int, pages int, page int, beginnum int, endnum int, style int) (output template.HTML) {
	...
	case style == 0: //mzr 定制版pagesbar
		...
		if results_max > 0 {
			raw = raw + "<span class='page-numbers'>共" + strconv.Itoa(int(pages)) + "页</span>"
			count := int(pages + 1)
			//prev page
			if (page != beginnum) && (page > beginnum) {
				prevpage = page - 1
				raw = raw + "<a class='prev page-numbers' href='" + url + keyword + "page-" + strconv.Itoa(prevpage) + "/'>Prev</a>"
			}
			...
			for i := page; i < count; i++ {
				...
				if i == page {
					...
					raw = raw + "<a class='page-numbers' href='" + url + keyword + "page-" + strconv.Itoa(i) + "/'>" + strconv.Itoa(i) + "</a>"
				}
				if j > pagewidth {
					break
				}
			...
			if (page != endnum) && (page < endnum) {
				nextpage = page + 1
				raw = raw + "<a class='next page-numbers' href='" + url + keyword + "page-" + strconv.Itoa(nextpage) + "/'>Next</a>"
			}
		} else {
...
func CheckPassword(password string) (b bool) {
	if ok, _ := regexp.MatchString("^[a-zA-Z0-9]{4,30}$", password); !ok {
		return false
	}
	return true
}
